...
namespace awin
{
    namespace platform::wayland
    {
                                         ...
                                         wl_fixed_t sx, wl_fixed_t sy)
        {
            ...
            if (!surface) return;
            if (wl_proxy_get_tag((wl_proxy *)surface) != &g_ctx->tag) return;

            WaylandWindowData *wl_data = (WaylandWindowData *)wl_surface_get_user_data(surface);

            g_ctx->serial = serial;
            g_ctx->pointer_enter_serial = serial;
            g_ctx->pointer_focus = wl_data;

            if (surface == wl_data->surface)
            {
                wl_data->hovered = true;
                if (wl_data->cursor) assign_cursor(wl_data, get_cursor_pd(wl_data->cursor));
                acul::events::dispatch_event_group<MouseEnterEvent>(g_env->events.mouse_enter, wl_data->owner, true);
            }
        ...
        {
            if (!surface) return;
            if (wl_proxy_get_tag((struct wl_proxy *)surface) != &g_ctx->tag) return;

            auto *wl_data = g_ctx->pointer_focus;
            if (!wl_data) return;

            g_ctx->serial = serial;
            g_ctx->pointer_focus = NULL;
            g_ctx->cursor_previous_name = NULL;

            if (wl_data->hovered)
            {
                wl_data->hovered = false;
                acul::events::dispatch_event_group<MouseEnterEvent>(g_env->events.mouse_enter, wl_data->owner, false);
            }
            else if (wl_data->fallback.decorations)
                wl_data->fallback.focus = NULL;
        }

        static void pointer_handle_motion(void *user_data, wl_pointer *pointer, u32 time, wl_fixed_t sx, wl_fixed_t sy)
        {
            auto *wl_data = g_ctx->pointer_focus;
            if (!wl_data) return;
            ...
            if (wl_data->fallback.decorations)
            {
                ...
                if (g_ctx->cursor_previous_name != cursor_name)
                {
                    wl_surface *surface = g_ctx->cursor_surface;
                    wl_cursor_theme *theme = g_ctx->cursor_theme;
                    int scale = 1;

                    if (wl_data->buffer_scale > 1 && g_ctx->cursor_theme_hi_dpi)
                    {
                        // We only support up to scale=2 for now, since libwayland-cursor
                        // requires us to load a different theme for each size.
                        scale = 2;
                        theme = g_ctx->cursor_theme_hi_dpi;
                    }
                    ...
                    wl_pointer_set_cursor(g_ctx->pointer, g_ctx->pointer_enter_serial, surface,
                    ...
                    g_ctx->cursor_previous_name = cursor_name;
                }
            }
        }
                                          ...
                                          u32 state)
        {
            ...
            if (wl_data->hovered)
            {
                ...
                acul::events::dispatch_event_group<MouseClickEvent>(
                    g_env->events.mouse_click, wl_data->owner, static_cast<io::MouseKey>(button - BTN_LEFT),
                    state == WL_POINTER_BUTTON_STATE_PRESSED ? io::KeyPressState::press : io::KeyPressState::release);
                return;
            }
                ...
                if (button == BTN_LEFT)
                {
                    ...
                    if (wl_data->fallback.focus == wl_data->fallback.top.surface)
                    {
                        if (wl_data->cursor_pos.y < AWIN_BORDER_SIZE)
                            ...
                            xdg_toplevel_move(wl_data->xdg.toplevel, g_ctx->seat, serial);
                    }
                    else if (wl_data->fallback.focus == wl_data->fallback.left.surface)
                    ...
                    if (edges != XDG_TOPLEVEL_RESIZE_EDGE_NONE)
                        xdg_toplevel_resize(wl_data->xdg.toplevel, g_ctx->seat, serial, edges);
                }
                else if (button == BTN_RIGHT && wl_data->xdg.toplevel)
                    xdg_toplevel_show_window_menu(wl_data->xdg.toplevel, g_ctx->seat, serial, wl_data->cursor_pos.x,
        ...
        {
            auto *window_data = g_ctx->pointer_focus;
            if (!window_data) return;

            // NOTE: 10 units of motion per mouse wheel step seems to be a common ratio
            if (axis == WL_POINTER_AXIS_HORIZONTAL_SCROLL)
                acul::events::dispatch_event_group<ScrollEvent>(g_env->events.scroll, window_data->owner,
                                                                -wl_fixed_to_double(value) / 10.0, 0.0);
            else if (axis == WL_POINTER_AXIS_VERTICAL_SCROLL)
                acul::events::dispatch_event_group<ScrollEvent>(g_env->events.scroll, window_data->owner, 0.0,
        ...
        static void keyboard_handle_keymap(void *user_data, wl_keyboard *keyboard, u32 format, int fd, u32 size)
        {
            ...
            keymap = xkb_keymap_new_from_string(g_ctx->xkb.context, map_str, XKB_KEYMAP_FORMAT_TEXT_V1,
            ...
            compose_table = xkb_compose_table_new_from_locale(g_ctx->xkb.context, locale, XKB_COMPOSE_COMPILE_NO_FLAGS);
            if (compose_table)
            {
            ...
            g_ctx->xkb.keymap = keymap;
            g_ctx->xkb.state = state;

            g_ctx->xkb.control_index = xkb_keymap_mod_get_index(g_ctx->xkb.keymap, "Control");
            g_ctx->xkb.alt_index = xkb_keymap_mod_get_index(g_ctx->xkb.keymap, "Mod1");
            g_ctx->xkb.shift_index = xkb_keymap_mod_get_index(g_ctx->xkb.keymap, "Shift");
            g_ctx->xkb.super_index = xkb_keymap_mod_get_index(g_ctx->xkb.keymap, "Mod4");
            g_ctx->xkb.caps_lock_index = xkb_keymap_mod_get_index(g_ctx->xkb.keymap, "Lock");
            g_ctx->xkb.num_lock_index = xkb_keymap_mod_get_index(g_ctx->xkb.keymap, "Mod2");
        }

        static void relative_pointer_handle_motion(void *user_data, zwp_relative_pointer_v1 *, u32 time_hi, u32 time_lo,
        ...
        inline void mark_focus_window(WaylandWindowData *window)
        {
            ...
            window->relative_pointer =
                zwp_relative_pointer_manager_v1_get_relative_pointer(g_ctx->relative_pointer_manager, g_ctx->pointer);
        ...
        {
            auto *window_data = g_ctx->keyboard_focus;
            if (!window_data) return;

            itimerspec timer = {{0}};
            timerfd_settime(g_ctx->key_repeat_timer_fd, 0, &timer, NULL);

            g_ctx->serial = serial;
            g_ctx->keyboard_focus = NULL;
            unmark_focus_window(window_data);
        }

        static xkb_keysym_t compose_symbol(xkb_keysym_t sym)
        {
            if (sym == XKB_KEY_NoSymbol || !g_ctx->xkb.compose_state) return sym;
            if (xkb_compose_state_feed(g_ctx->xkb.compose_state, sym) != XKB_COMPOSE_FEED_ACCEPTED) return sym;
            switch (xkb_compose_state_get_status(g_ctx->xkb.compose_state))
            {
                case XKB_COMPOSE_COMPOSED:
                    return xkb_compose_state_get_one_sym(g_ctx->xkb.compose_state);
        ...
        static void input_text(WindowData *window_data, u32 scancode)
        {
            ...
            if (xkb_state_key_get_syms(g_ctx->xkb.state, keycode, &keysyms) == 1)
            {
                const xkb_keysym_t keysym = compose_symbol(keysyms[0]);
                const u32 codepoint = xkb_keysym_to_utf32(keysym);
                if (codepoint != 0)
                    acul::events::dispatch_event_group<CharInputEvent>(g_env->events.char_input, window_data->owner,
                                                                       codepoint);
            }
        }
                                        ...
                                        u32 state)
        {
            ...
            if (action == io::KeyPressState::press)
            {
                ...
                if (xkb_keymap_key_repeats(g_ctx->xkb.keymap, keycode) && g_ctx->key_repeat_rate > 0)
                {
                    g_ctx->key_repeat_scancode = scancode;
                    if (g_ctx->key_repeat_rate > 1)
                        timer.it_interval.tv_nsec = 1000000000 / g_ctx->key_repeat_rate;
                    else
                        timer.it_interval.tv_sec = 1;

                    timer.it_value.tv_sec = g_ctx->key_repeat_delay / 1000;
                    timer.it_value.tv_nsec = (g_ctx->key_repeat_delay % 1000) * 1000000;
                }
            }

            timerfd_settime(g_ctx->key_repeat_timer_fd, 0, &timer, NULL);
            const auto key = g_ctx->keymap.find(scancode);
            io::KeyMode mods = g_ctx->xkb.modifiers;
        ...
        static void keyboard_handle_modifiers(void *user_data, wl_keyboard *keyboard, u32 serial, u32 mods_depressed,
                                              u32 mods_latched, u32 mods_locked, u32 group)
        {
            g_ctx->serial = serial;

            if (!g_ctx->xkb.keymap) return;
            xkb_state_update_mask(g_ctx->xkb.state, mods_depressed, mods_latched, mods_locked, 0, 0, group);
            ...
            struct
            {
                ...
                {g_ctx->xkb.control_index, io::KeyModeBits::control},
                {g_ctx->xkb.alt_index, io::KeyModeBits::alt},
                {g_ctx->xkb.shift_index, io::KeyModeBits::shift},
                {g_ctx->xkb.super_index, io::KeyModeBits::super},
                {g_ctx->xkb.caps_lock_index, io::KeyModeBits::caps_lock},
                {g_ctx->xkb.num_lock_index, io::KeyModeBits::num_lock},
            };

            for (size_t i = 0; i < sizeof(modifiers) / sizeof(modifiers[0]); i++)
                if (xkb_state_mod_index_is_active(g_ctx->xkb.state, modifiers[i].index, XKB_STATE_MODS_EFFECTIVE) == 1)
                    g_ctx->xkb.modifiers |= modifiers[i].bit;
        }

        static void keyboard_handle_repeat_info(void *user_data, wl_keyboard *keyboard, i32 rate, i32 delay)
        {
            if (keyboard != g_ctx->keyboard) return;

            g_ctx->key_repeat_rate = rate;
            g_ctx->key_repeat_delay = delay;
        }
        ...
        {
            if ((caps & WL_SEAT_CAPABILITY_POINTER) && !g_ctx->pointer)
            {
                g_ctx->pointer = wl_seat_get_pointer(seat);
                wl_pointer_add_listener(g_ctx->pointer, &pointer_listener, NULL);
            }
            else if (!(caps & WL_SEAT_CAPABILITY_POINTER) && g_ctx->pointer)
            {
                wl_pointer_destroy(g_ctx->pointer);
                g_ctx->pointer = NULL;
            }

            if ((caps & WL_SEAT_CAPABILITY_KEYBOARD) && !g_ctx->keyboard)
            {
                g_ctx->keyboard = wl_seat_get_keyboard(seat);
                wl_keyboard_add_listener(g_ctx->keyboard, &keyboard_listener, NULL);
            }
            else if (!(caps & WL_SEAT_CAPABILITY_KEYBOARD) && g_ctx->keyboard)
            {
                wl_keyboard_destroy(g_ctx->keyboard);
                g_ctx->keyboard = NULL;
            }
        }
        ...
        {
            g_ctx->offers.emplace_back(offer);
            wl_data_offer_add_listener(offer, &data_offer_listener, nullptr);
        }

        static void data_device_handle_enter(void *user_data, wl_data_device *device, u32 serial, wl_surface *surface,
        ...
        static void data_device_handle_selection(void *user_data, wl_data_device *device, wl_data_offer *offer)
        {
            ...
            {
                wl_data_offer_destroy(g_ctx->selection_offer);
                g_ctx->selection_offer = NULL;
            }

            for (unsigned int i = 0; i < g_ctx->offers.size(); i++)
            {
                if (g_ctx->offers[i].offer == offer)
                {
                    if (g_ctx->offers[i].text_plain_utf8)
                        g_ctx->selection_offer = offer;
                    else
                        wl_data_offer_destroy(offer);

                    g_ctx->offers[i] = g_ctx->offers.back();
                    g_ctx->offers.pop_back();
                    break;
                }
            }
        }
                                                     ...
                                                     u32 numerator)
        {
            ...
            acul::events::dispatch_event_group<DpiChangedEvent>(g_env->events.dpi_changed, window->owner, dpi, dpi);
        }
        ...
        static bool resize_window(WaylandWindowData *window, acul::point2D<int> dimensions)
        {
            ...
            if (window->fallback.decorations)
            {
                wp_viewport_set_destination(window->fallback.top.viewport, window->dimenstions.x, AWIN_CAPTION_HEIGHT);
        ...
        void libdecor_frame_handle_configure(libdecor_frame *frame, libdecor_configuration *config, void *user_data)
        {
            ...
            if (!libdecor_configuration_get_content_size(config, frame, &size.x, &size.y)) size = window->dimenstions;
        ...
        {
            if (enable && !wl_data->idle_inhibitor && g_ctx->idle_inhibit_manager)
            {
                                         ...
                                         wl_buffer *buffer, acul::point2D<int> pos, acul::point2D<int> size)
        {
            ...
            wl_proxy_set_tag((struct wl_proxy *)edge->surface, &g_ctx->tag);
            edge->subsurface = wl_subcompositor_get_subsurface(g_ctx->subcompositor, edge->surface, parent);
            wl_subsurface_set_position(edge->subsurface, pos.x, pos.y);
            edge->viewport = wp_viewporter_get_viewport(g_ctx->viewporter, edge->surface);
            wp_viewport_set_destination(edge->viewport, size.x, size.y);
            wl_surface_attach(edge->surface, buffer, 0, 0);

            wl_region *region = wl_compositor_create_region(g_ctx->compositor);
        ...
        static wl_buffer *create_shm_buffer(const Image *image)
        {
            ...
            struct wl_shm_pool *pool = wl_shm_create_pool(g_ctx->shm, fd, length);
            ...
            wl_buffer *buffer = wl_shm_pool_create_buffer(pool, 0, image->dimenstions.x, image->dimenstions.y, stride,
        ...
        static void xdg_decoration_handle_configure(void *user_data, zxdg_toplevel_decoration_v1 *decoration, u32 mode)
        {
            WaylandWindowData *window = (WaylandWindowData *)user_data;
        ...
        static bool create_shell_objects(WaylandWindowData *wl_data)
        {
            ...
            if (g_ctx->libdecor.context)
            {
                if (create_libdecor_frame(wl_data))
                {
                    ...
                    if (g_ctx->is_surface_placeholder_enabled)
                    {
            ...
            if (g_ctx->decoration_manager)
            {
                wl_data->xdg.decoration = zxdg_decoration_manager_v1_get_toplevel_decoration(g_ctx->decoration_manager,
                                                                                             wl_data->xdg.toplevel);
                zxdg_toplevel_decoration_v1_add_listener(wl_data->xdg.decoration, &xdg_decoration_listener, wl_data);

                u32 mode = wl_data->flags & WindowFlagBits::decorated ? ZXDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE
        ...
        {
            if (wl_proxy_get_tag((struct wl_proxy *)output) != &g_ctx->tag) return;
        ...
        {
            if (wl_proxy_get_tag((wl_proxy *)output) != &g_ctx->tag) return;
        ...
        bool create_window(WindowData *window_data, const acul::string &title, i32 width, i32 height, WindowFlags flags)
        {
            ...
            if (g_ctx->fractional_scale_manager && wl_data->scale_framebuffer)
            {
                wl_data->scaling_viewport = wp_viewporter_get_viewport(g_ctx->viewporter, wl_data->surface);
                wp_viewport_set_destination(wl_data->scaling_viewport, wl_data->dimenstions.x, wl_data->dimenstions.y);
                wl_data->fractional_scale = wp_fractional_scale_manager_v1_get_fractional_scale(
                    g_ctx->fractional_scale_manager, wl_data->surface);
        ...
        {
            while (wl_display_flush(g_ctx->display) == -1)
            {
                if (errno != EAGAIN) return false;

                pollfd fd = {wl_display_get_fd(g_ctx->display), POLLOUT};
        ...
        static void set_cursor_image(WaylandWindowData *wl_data, WaylandCursor *wl_cursor)
        {
            ...
            else
            {
                ...
                timerfd_settime(g_ctx->cursor_timer_fd, 0, &timer, NULL);
            ...
            wl_pointer_set_cursor(g_ctx->pointer, g_ctx->pointer_enter_serial, surface, wl_cursor->hot.x / scale,
        ...
        static void handle_events(f64 *timeout)
        {
            ...
            pollfd fds[] = {{wl_display_get_fd(g_ctx->display), POLLIN},
                            {g_ctx->key_repeat_timer_fd, POLLIN},
                            {g_ctx->cursor_timer_fd, POLLIN},
                            {-1, POLLIN}};

            if (g_ctx->libdecor.context) fds[LIBDECOR_FD].fd = libdecor_get_fd(g_ctx->libdecor.context);

            while (!event)
            {
                while (wl_display_prepare_read(g_ctx->display) != 0)
                    if (wl_display_dispatch_pending(g_ctx->display) > 0) return;
                ...
                if (fds[DISPLAY_FD].revents & POLLIN)
                {
                    ...
                    if (wl_display_dispatch_pending(g_ctx->display) > 0) event = true;
                }
                else
                    wl_display_cancel_read(g_ctx->display);
                ...
                if (fds[KEYREPEAT_FD].revents & POLLIN)
                {
                    ...
                    if (read(g_ctx->key_repeat_timer_fd, &repeats, sizeof(repeats)) == 8)
                    {
                        if (g_ctx->keyboard_focus)
                        {
                            const auto key = g_ctx->keymap.find(g_ctx->key_repeat_scancode);
                            for (u64 i = 0; i < repeats; i++)
                            {
                                input_key(g_ctx->keyboard_focus, key, io::KeyPressState::press, g_ctx->xkb.modifiers);
                                input_text(g_ctx->keyboard_focus, g_ctx->key_repeat_scancode);
                            }
                ...
                {
                    u64 repeats;
                    if (read(g_ctx->cursor_timer_fd, &repeats, sizeof(repeats)) == 8)
                        increment_cursor_image(g_ctx->pointer_focus);
                }
            }

            if (fds[LIBDECOR_FD].revents & POLLIN)
            {
                if (libdecor_dispatch(g_ctx->libdecor.context, 0) > 0) event = true;
            }
        }
        ...
        static void data_source_handle_send(void *user_data, wl_data_source *source, const char *mime_type, int fd)
        {
            ...
            if (g_ctx->selection_source != source || strcmp(mime_type, "text/plain;charset=utf-8") != 0)
            {
                close(fd);
                return;
            }

            char *string = (char *)g_env->clipboard_data.c_str();
            size_t length = g_env->clipboard_data.length();
        ...
        static void data_source_handle_cancelled(void *user_data, wl_data_source *source)
        {
            ...
            if (g_ctx->selection_source != source) return;
            g_ctx->selection_source = NULL;
        }
        ...
        acul::string get_clipboard_string()
        {
            ...
            if (g_ctx->selection_source) return g_env->clipboard_data;
            g_env->clipboard_data = read_data_offer_as_string(g_ctx->selection_offer, "text/plain;charset=utf-8");
            return g_env->clipboard_data;
        }
        ...
        void set_clipboard_string(const acul::string &text)
        {
            ...
            {
                wl_data_source_destroy(g_ctx->selection_source);
                g_ctx->selection_source = NULL;
            }

            g_env->clipboard_data = text;
            ...
            wl_data_source_offer(g_ctx->selection_source, "text/plain;charset=utf-8");
            wl_data_device_set_selection(g_ctx->data_device, g_ctx->selection_source, g_ctx->serial);
        }
        ...
        Cursor::Platform *create_cursor(Cursor::Type type)
        {
            ...
            cursor->handle = wl_cursor_theme_get_cursor(g_ctx->cursor_theme, entry.name);
...
#ifdef AWIN_TEST_BUILD
        void enable_wayland_surface_placeholder() { platform::wayland::g_ctx->is_surface_placeholder_enabled = true; }
#endif
    } // namespace native_access
} // namespace awin