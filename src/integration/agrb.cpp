...
namespace awin::integration
{
    void CreateCtx::assign_instance_extensions(const acul::set<acul::string> &ext, acul::vector<const char *> &dst)
    {
        ...
        if (backend == WINDOW_BACKEND_X11)
        {
            ...
            if (it != ext.end())
            {
                platform::x11::g_ctx->xlib.xcb.is_extension_present = true;
                                          ...
                                          vk::DispatchLoaderDynamic &loader, Window &window)
    {
        ...
        info.setDpy(platform::x11::g_ctx->display).setWindow(native_access::get_x11_window_handle(window));
                                         ...
                                         vk::DispatchLoaderDynamic &loader)
    {
        try
        {
            ...
            else if (backend == WINDOW_BACKEND_WAYLAND)
            {
                ...
                info.setDisplay(platform::wayland::g_ctx->display)
